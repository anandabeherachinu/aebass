 			...
 			<c:when test="${sessionScope.role_id eq '1' ||sessionScope.role_id eq '3' }">
				...
				<thead>
					...
					<tbody>
						...
						<c:forEach var="s" items="${branchrwisecount}" varStatus="count">
							<tr>
									<td>${count.index+1}</td>
									<td class="top-buffer">${s.instituteName}</td>  
									<td class="top-buffer">${s.instituteCode}</td> 
					<td><a href="view-institute-branchwise-studentname-count.html?instituteName=${s.instituteName}&instituteCode=${s.instituteCode}&courseName=${s.courseName}"  >${s.courseName}</a></td>
