...
<%--   <div class="panel-heading">
 			...
 			<div class="box col-md-12">
 				...
 				<form:form class="form-horizontal" action="view-all-student-list-sidebar.html" method="post"  modelAttribute="b"
 				 autocomplete="off" id="attTc">
 				
				<table class="table_form">
 						<th colspan="2">Institute List</th>
 					
 					
 					<tr>
						<!-- <td><label id="secetor-text">Select PIA:<span class="required-star">*</span></label></td> -->
						<td id="secetor-text">Select Institute Type:<span class="required-star">*</span></td>
						
						<td>
							 <form:select path="instituteCode" class="courese-sector" required="true" id="piaAjax" >
					      			<form:option value="">Select</form:option>
					      			<c:forEach var="s"  items="${collegetype}">
					      				<form:option value="${s.instituteCode}">${s.instituteName}</form:option>		
					      			</c:forEach>
 			...
 			<c:when test="${sessionScope.role_id eq '1' }">
				...
				<thead>
					...
					<tbody>
						...
						<c:forEach var="s" items="${listOfStudent}" varStatus="count">
							<tr>
									<td>${count.index+1}</td>
				<td class="top-buffer">${s.rollId}</td>  
				<td>${s.studentName}</td>
